...
const requestAuthUser = async (req: IncomingMessage): Promise<{ userId: string; username: string; isLoggedIn: boolean }> => {
  try {
    const { data } = await axios.get("/auth", { headers: { cookie: req.headers.cookie } });
    const authUser = { userId: data.userId, username: data.username, isLoggedIn: true };
    return authUser;
  } catch (error) {
    throw error;
  }
...
const getAuthUser = async (
  req: IncomingMessage,
  res: ServerResponse
): Promise<{ userId: string; username: string; isLoggedIn: boolean }> => {
  try {
    ...
    if ((error as any)?.response?.status === 401) {
      ...
      try {
        ...
        return { userId: "", username: "", isLoggedIn: false };
      }
    }
